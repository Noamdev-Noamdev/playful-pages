...
export function QuantityViz({ data, animated }: { data: QuantityVizData; animated: boolean }) {
  ...
  const dotsA =
    data.countA === larger ? MAX_DOTS : Math.max(1, Math.round((data.countA / larger) * MAX_DOTS));
  const dotsB =
    data.countB === larger ? MAX_DOTS : Math.max(1, Math.round((data.countB / larger) * MAX_DOTS));
          ...
          return (
            ...
            >
              ...
              >
                ...
                <p className="text-[9px] text-muted-foreground whitespace-nowrap">{ev.display}</p>
              </div>
            </div>
          );
        })}
        ...
        >
          {sorted.slice(1).map((ev, i) => {
            ...
            return (
              <div
                key={i}
                className="text-[11px] text-muted-foreground bg-muted px-2 py-1 rounded-lg"
              >
