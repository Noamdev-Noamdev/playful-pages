        ...
        <svg viewBox="0 0 480 260" width="100%" height="100%" style={{ display: "block" }}>
            <defs>
                <radialGradient id="smogGrad">
                    ...
                    <stop offset="100%" stopColor={`rgb(${Math.round(lerp(80, 50, smogT))},${Math.round(lerp(80, 55, smogT))},${Math.round(lerp(80, 50, smogT))})`} />
                </radialGradient>
            </defs>
            ...
            {/* Smog layer */}
            {smogT > 0 && (
                <rect x="0" y="0" width="480" height={lerp(0, 100, smogT)}
                    fill={`rgba(120,100,80,${smogT * 0.5})`}
                    style={{ transition: "all 2s" }} />
            )}

            {/* Ground */}
            <rect x="0" y="200" width="480" height="60" fill={`rgb(${Math.round(lerp(80, 55, smogT))},${Math.round(lerp(90, 65, smogT))},${Math.round(lerp(70, 50, smogT))})`}
                style={{ transition: "fill 2s" }} />
            ...
            {t > 0.1 && (
                <>
                    ...
                    {[60, 120, 180, 300, 360, 420].map(x => (
                        <line key={x} x1={x} y1="204" x2={x + 40} y2="204" stroke="#6b7280" strokeWidth="1" strokeDasharray="6 4" />
                    ))}
                </>
            )}
            ...
            {[
                ...
                <g key={i}>
                    ...
                    {b.h > 25 && Array.from({ length: Math.floor(b.h / 14) }, (_, r) =>
                        Array.from({ length: Math.floor(b.w / 9) }, (_, c) => (
                            <rect key={`${r}-${c}`}
                                x={b.x + 3 + c * 9} y={203 - b.h + r * 12}
                                width={4} height={7} rx="1"
            ...
            {/* Crisis: dark rain */}
            {crisisT > 0 && Array.from({ length: 25 }, (_, i) => (
                <line key={i}
                    x1={10 + (i * 43 + progress * 6) % 470}
                    y1={(i * 37) % 190}
                    x2={5 + (i * 43 + progress * 6) % 470}
                    y2={18 + (i * 37) % 190}
                    stroke="#78350f" strokeWidth="1" opacity={crisisT * 0.6}
                />
            ))}

            {/* Factories right side */}
            {t > 0.25 && [340, 390, 430].map((x, i) => (
    ...
    return (
        <svg viewBox="0 0 480 260" width="100%" height="100%" style={{ display: "block" }}>
            ...
            <rect width="480" height="260"
                fill={`rgb(${skyR},${skyG},${skyB})`} style={{ transition: "fill 2s" }} />

            {/* Incident warning glow */}
            {incidentT > 0 && recoveryT < 0.5 && (
                <circle cx="160" cy="120" r={lerp(0, 200, incidentT)}
                    fill={`rgba(250,204,21,${incidentT * 0.15})`} />
            )}

            {/* Ground */}
            <rect x="0" y="198" width="480" height="62"
                fill={`rgb(${Math.round(lerp(100, 80, t))},${Math.round(lerp(140, 120, t))},${Math.round(lerp(80, 65, t))})`}
                style={{ transition: "fill 2s" }} />

            {/* River / cooling water */}
            <path d="M0,215 Q120,210 240,218 Q360,225 480,212" fill="none"
                stroke="#60a5fa" strokeWidth="10" opacity="0.6" />

            {/* Cooling towers */}
            {[80, 140].map((x, i) => (
                <g key={i} opacity={Math.min(1, (progress - 2) / 10)}>
                    {/* Tower shape */}
                    <path d={`M${x - 18},200 Q${x},${165 - i * 5} ${x + 18},200 Q${x + 8},${175 - i * 5} ${x},${163 - i * 5} Q${x - 8},${175 - i * 5} ${x - 18},200 Z`}
            ...
            {domeH > 0 && (
                <g opacity={Math.min(1, (progress - 5) / 8)}>
                    ...
                    <path d={`M118,198 Q118,${198 - 55 * domeH} 160,${198 - 60 * domeH} Q202,${198 - 55 * domeH} 202,198 Z`}
            ...
            {[
                ...
                <g key={i}>
                    ...
                    {b.h > 22 && Array.from({ length: Math.floor(b.h / 13) }, (_, r) =>
                        Array.from({ length: Math.floor(b.w / 9) }, (_, c) => (
                            <rect key={`${r}-${c}`}
                                x={b.x + 3 + c * 9} y={203 - b.h + r * 11}
            ...
            {recoveryT > 0 && [350, 380, 410, 440].map((x, i) => (
...
function RenewablesScene({ progress }: { progress: number }) {
    ...
    return (
        <svg viewBox="0 0 480 260" width="100%" height="100%" style={{ display: "block" }}>
            ...
            {surplusGlow > 0 && (
                <circle cx="420" cy="40" r={30 + surplusGlow * 20}
                    fill={`rgba(253,230,138,${surplusGlow * 0.3})`} />
            )}

            {/* Clouds — white and clean */}
            {[0, 1, 2].map(i => (
                <ellipse key={i} cx={60 + i * 160} cy={30 + i * 8}
                    rx={35 + i * 10} ry={15 + i * 3}
                    fill="rgba(255,255,255,0.9)" />
            ))}

            {/* Rolling hills */}
            <path d="M0,200 Q80,160 160,190 Q240,220 320,185 Q400,150 480,190 L480,260 L0,260 Z"
                fill={`rgb(${Math.round(lerp(100, 60, t * 0.5))},${Math.round(lerp(160, 130, t * 0.3))},${Math.round(lerp(70, 50, t * 0.3))})`}
                style={{ transition: "fill 2s" }} />
            <path d="M0,210 Q80,180 160,205 Q240,230 320,200 Q400,170 480,205 L480,260 L0,260 Z"
                fill={`rgb(${Math.round(lerp(80, 55, t * 0.4))},${Math.round(lerp(140, 110, t * 0.3))},${Math.round(lerp(60, 45, t * 0.3))})`} />

            {/* Ground */}
            <rect x="0" y="210" width="480" height="50"
            ...
            {Array.from({ length: numPanels }, (_, i) => {
                ...
                return (
                    <g key={i} opacity={Math.min(1, (progress - 20 - i * 4) / 8)}>
                        <rect x={px} y={py} width="22" height="12" rx="1"
                            fill="#1e40af" stroke="#1d4ed8" strokeWidth="0.5"
                            transform={`rotate(-8,${px + 11},${py + 6})`} />
                        <line x1={px + 8} y1={py + 12} x2={px + 8} y2={py + 16} stroke="#6b7280" strokeWidth="1.5" />
                        <line x1={px + 14} y1={py + 12} x2={px + 14} y2={py + 16} stroke="#6b7280" strokeWidth="1.5" />
                        {/* Panel gleam */}
                        <line x1={px + 4} y1={py + 3} x2={px + 8} y2={py + 1}
            ...
            {[
                ...
                <g key={i}>
                    ...
                    {b.h > 20 && Array.from({ length: Math.floor(b.h / 13) }, (_, r) =>
                        Array.from({ length: Math.floor(b.w / 9) }, (_, c) => (
                            <rect key={`${r}-${c}`}
                                x={b.x + 3 + c * 9} y={215 - b.h + r * 11}
                                width={4} height={6} rx="1"
