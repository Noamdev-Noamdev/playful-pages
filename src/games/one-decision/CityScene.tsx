...
function CoastScene({ progress }: { progress: number }) {
  ...
  return (
    <svg viewBox="0 0 480 260" width="100%" height="100%" style={{ display: "block" }}>
      ...
      {t > 0.08 && (
        <g
          transform={`translate(${60 + ((progress * 3.2) % 280)}, 170)`}
      ...
      <g clipPath="url(#buildingClip)">
        {buildings.map((b, i) => (
          <g key={i}>
            ...
            {/* Windows */}
            {b.h > 20 &&
              Array.from({ length: Math.floor(b.h / 16) }, (_, r) =>
  ...
  return (
    <svg viewBox="0 0 480 260" width="100%" height="100%" style={{ display: "block" }}>
      ...
      <rect
        ...
        width="480"
        height="72"
        fill={`rgb(${Math.round(lerp(140, 100, greenT))},${Math.round(lerp(180, 140, greenT))},${Math.round(lerp(100, 80, greenT))})`}
        style={{ transition: "fill 2s" }}
      />

      {/* River */}
      <path
        d="M480,200 Q380,195 300,205 Q220,215 140,205 Q80,198 0,202"
        fill="none"
        stroke="#60a5fa"
        strokeWidth="8"
        opacity="0.8"
      ...
      {[60, 100, 140, 350, 390, 420].map((x, i) => {
        ...
        return (
          <g key={i} opacity={treeT}>
            ...
            <polygon
              points={`${x},${182 - lerp(0, 22, treeT)} ${x - 10},194 ${x + 10},194`}
  ...
  return (
    <svg viewBox="0 0 480 260" width="100%" height="100%" style={{ display: "block" }}>
      ...
      <rect
        ...
        width="480"
        height="65"
        fill={`rgb(${Math.round(lerp(210, 180, t))},${Math.round(lerp(150, 110, t))},${Math.round(lerp(60, 40, t))})`}
        style={{ transition: "fill 2s" }}
      />
      ...
      {oilT > 0 &&
        [300, 340, 370].map((x, i) => (
          <g key={i} opacity={Math.min(1, oilT * 2 - i * 0.3)}>
            <polygon points={`${x},${185 - 30} ${x - 10},185 ${x + 10},185`} fill="#1f2937" />
            <line x1={x} y1="185" x2={x} y2={185 - 30} stroke="#374151" strokeWidth="3" />
            {/* Pumping beam */}
            <line
              x1={x - 8}
              y1={185 - 25 + Math.sin(progress * 0.15 + i) * 5}
              x2={x + 8}
              y2={185 - 18 - Math.sin(progress * 0.15 + i) * 5}
              stroke="#6b7280"
              strokeWidth="3"
            />
          </g>
      ...
      {/* Solar panels */}
      {solarT > 0 &&
        [60, 100, 140, 340, 380, 420].map((x, i) => (
