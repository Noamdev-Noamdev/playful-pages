...
function statLabel(key: string): string {
    return { population: "👥 Population", economy: "💰 Economy", environment: "🌿 Environment" }[key] ?? key;
}
...
export function DecisionGame() {
    ...
    const startSim = useCallback((ch: Choice) => {
        setChoice(ch);
        setPhase("simulating");
                ...
                for (const ev of ch.events) {
                    if (!newFired.has(ev.id) && newProgress >= ev.progress) {
                        ...
                        if (ev.changes.population !== undefined)
                            newStats.population = Math.min(100, Math.max(0, newStats.population + ev.changes.population * jitter()));
                        if (ev.changes.economy !== undefined)
                            newStats.economy = Math.min(100, Math.max(0, newStats.economy + ev.changes.economy * jitter()));
                        if (ev.changes.environment !== undefined)
                            newStats.environment = Math.min(100, Math.max(0, newStats.environment + ev.changes.environment * jitter()));

                        showToast(ev.title, ev.description);
                    }
                }

                return { ...prev, progress: newProgress, stats: newStats, firedEventIds: newFired, latestEvent };
            });
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-3">
                    {scenario.choices.map((ch) => (
                        <button key={ch.id} onClick={() => startSim(ch)}
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            {/* SVG Scene */}
            <div className="rounded-3xl border-2 border-foreground overflow-hidden"
            ...
            {/* Event toast */}
            {toast && (
                <div className="rounded-2xl border-2 border-foreground bg-card px-4 py-3
                ...
                <div className="rounded-2xl border-2 border-foreground bg-card px-5 py-5 flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        {(["population", "economy", "environment"] as const).map((key) => {
                            const v = Math.round(gameState.stats[key]);
                            return (
                                <div key={key} className="rounded-xl border border-foreground/30 bg-muted px-2 py-2 text-center">
                    ...
                    {/* Replay buttons */}
                    <div className="flex gap-3">
                        <button onClick={() => reset(scenarioIdx)}
                            className="flex-1 py-2.5 rounded-xl border-2 border-foreground bg-card
                text-sm font-semibold text-foreground hover:bg-foreground hover:text-background transition-colors">
                            Try another choice
                        </button>
