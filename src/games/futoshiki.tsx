...
function deepCopyGrid(g: FGrid): FGrid {
  return g.map((r) => [...r]);
}
...
function generatePuzzle(): { puzzle: FGrid; solution: FGrid; ineqs: FIneq[] } {
  while (true) {
    ...
    const ineqs: FIneq[] = chosen.map(([r1, c1, r2, c2]) => {
      return sol[r1][c1]! < sol[r2][c2]! ? { r1, c1, r2, c2 } : { r1: r2, c1: c2, r2: r1, c2: c1 };
    });

    // 4. Verify the solution still satisfies all constraints
    let valid = true;
    for (const { r1, c1, r2, c2 } of ineqs)
      if (sol[r1][c1]! >= sol[r2][c2]!) {
        valid = false;
        break;
      }
    if (!valid) continue;
...
function FutoshikiGame() {
  const [puzzle, setPuzzle] = useState<FGrid>(() =>
    Array.from({ length: N }, () => Array(N).fill(null)),
  );
  const [solution, setSolution] = useState<FGrid>(() =>
    Array.from({ length: N }, () => Array(N).fill(null)),
  );
  const [userGrid, setUserGrid] = useState<FGrid>(() =>
    Array.from({ length: N }, () => Array(N).fill(null)),
  );
    ...
    for (let gj = 0; gj < 9; gj++) {
      ...
      if (isCell) {
        ...
        const err =
          ...
          (sign === "<" ? isIneqError(r, c, r, c + 1) : isIneqError(r, c + 1, r, c));
        ...
        gridItems.push(
          ...
          >
            {sign && (
              <span
                style={{ fontSize: "0.85rem", fontWeight: 800, color: err ? "#ef4444" : "#a78bfa" }}
              >
                {sign}
              </span>
            )}
        ...
        const err =
          sign && (sign === "∨" ? isIneqError(r, c, r + 1, c) : isIneqError(r + 1, c, r, c));
  ...
  return (
    <div className="flex flex-col items-center gap-5 py-6 select-none">
      <WinOverlay
        ...
        sub="All inequalities satisfied!"
      />

      <p className="text-xs text-slate-400 max-w-xs text-center">
        Fill each row and column with <strong>1–5</strong> (no repeats). Inequality signs between
        cells must be respected. <strong>∨</strong> = upper cell is smaller, <strong>∧</strong> =
        upper cell is larger.
      </p>
